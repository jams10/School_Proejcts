#! /usr/bin/env python
#!는 Shebang. 매직넘버로 이 스크립트를 실행시켜 줄 프로그램(런쳐)의 경로를 지정해줌.
# 프로그램 경로는 시스템 환경에 따라 달라지기 때문에 env를 써줌.
# env는 환경 변수에서 지정한 언어의 위치를 찾아 실행됨.
...
class scheduler:
    # scheduler 클래스 생성자(인스턴스 생성 시점에 호출).
    # 프로그램에서 1)프로세스의 전환 시기, 2)입출력 완료후 프로세스 전환 방식, 3)입출력 수행 시간 을 설정.
    def __init__(self, process_switch_behavior, io_done_behavior, io_length): 
        ...
        return
    # 새 프로세스 생성. 프로세스가 생성되면 READY 상태로 초기화.
...
parser.add_option('-l', '--processlist', default='',
                  help='a comma-separated list of processes to run, in the form X1:Y1,X2:Y2,\
                  ... where X is the number of instructions that process should run,\ 
                  and Y the chances (from 0 to 100) that an instruction will use the CPU or issue an IO', 
...
parser.add_option('-p', '--printstats', help='print statistics at end;\ 
                  only useful with -c flag (otherwise stats are not printed)', 
